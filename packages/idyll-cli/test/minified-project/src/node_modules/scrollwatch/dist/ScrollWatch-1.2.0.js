(function(root, factory) {
	...
	var saveContainerElement = function() {
		if (!isContainerWindow.call(this)) {
			instanceData[this._id].config.container = document.querySelector(
				instanceData[this._id].config.container
			);
		}
	};
	...
	var saveElements = function() {
		instanceData[this._id].elements = Array.prototype.slice.call(
			document.querySelectorAll(
				instanceData[this._id].config.watch +
					':not(.' +
					instanceData[this._id].config.ignoreClass +
					')'
			)
		);
	};
	...
	var saveScrollPosition = function() {
		instanceData[this._id].lastScrollPosition = getScrollPosition.call(
			this
		);
	};
	...
	var checkElements = function(eventType) {
		...
		for (i = 0; i < len; i++) {
			...
			if (eventType === 'scroll') {
				responseData.direction = getScrolledDirection.call(
					this,
					getScrolledAxis.call(this)
				);
			}
	...
	var checkInfinite = function(eventType) {
		...
		if (config.infiniteScroll && !data.isInfiniteScrollPaused) {
			...
			for (i = 0; i < 2; i++) {
				...
				if (
					((eventType === 'scroll' &&
						hasScrollPositionChanged.call(this, axis[i])) ||
						eventType === 'resize' ||
						eventType === 'refresh' ||
						eventType === initEvent) &&
					viewableRange[axis[i]].end + config.infiniteOffset >=
						scrollSize[i]
				) {
	...
	var getScrollingElement = function() {
		return isContainerWindow.call(this)
			? window
			: instanceData[this._id].config.container;
	};
	...
	var getElementRange = function(el) {
		...
		if (isContainerWindow.call(this)) {
			...
			containerCoords = instanceData[
				this._id
			].config.container.getBoundingClientRect();

			range.x.start =
				coords.left - containerCoords.left + viewableRange.x.start;
			range.x.end = range.x.start + coords.width;

			range.y.start =
				coords.top - containerCoords.top + viewableRange.y.start;
	...
	var getScrolledDirection = function(axis) {
		...
		return curScrollPosition[position[axis]] >
			lastScrollPosition[position[axis]]
			? scrollDir[axis][0]
			: scrollDir[axis][1];
	};
	...
	var hasScrollPositionChanged = function(axis) {
		...
		return (
			curScrollPosition[position[axis]] !==
			lastScrollPosition[position[axis]]
		);
	};
	...
	var isElementInView = function(el) {
		...
		return (
			isElementInVerticalView(elRange, viewableRange, offset) &&
			isElementInHorizontalView(elRange, viewableRange, offset)
		);
	};

	var isElementInVerticalView = function(elRange, viewableRange, offset) {
		return (
			elRange.y.start < viewableRange.y.end + offset &&
			elRange.y.end > viewableRange.y.start - offset
		);
	};

	var isElementInHorizontalView = function(elRange, viewableRange, offset) {
		return (
			elRange.x.start < viewableRange.x.end + offset &&
			elRange.x.end > viewableRange.x.start - offset
		);
	};
	...
	var handler = function(e) {
		...
		if (
			eventType === 'resize' ||
			hasScrollPositionChanged.call(this, 'x') ||
			hasScrollPositionChanged.call(this, 'y')
		) {
	...
	var ScrollWatch = function(opts) {
		...
		if (this instanceof ScrollWatch) {
			...
			if (data.config.debounce) {
				data.scrollHandler = debounce(
					handler.bind(this),
					data.config.scrollDebounce,
					data.config.debounceTriggerLeading
				);
				data.resizeHandler = debounce(
					handler.bind(this),
					data.config.resizeDebounce,
					data.config.debounceTriggerLeading
				);
			} else {
				data.scrollHandler = throttle(
					handler.bind(this),
					data.config.scrollThrottle,
					this
				);
				data.resizeHandler = throttle(
					handler.bind(this),
					data.config.resizeThrottle,
					this
				);
			}
